...
document.addEventListener("milkJavaSciptEvent", function(event) {
    document.body.innerHTML = event.detail.id;
});

document.addEventListener("milkPythonEvent", function(event) {
...
class App {
    exit() {
      return milk._call_api({
        event: 'app.exit'
      })
    }
    set_full_screen() {
      return milk._call_api({
        event: 'app.set_full_screen'
      })
    }
    exit_full_screen() {
      return milk._call_api({
        event: 'app.exit_full_screen'
      })
    }
    toggle_full_screen() {
      return milk._call_api({
        event: 'app.toggle_full_screen'
      })
    }
}

class Dialog {
    info(title='Milk', message='Hello World') {
        return milk._call_api({
            event: 'dialog.info',
            params: {
                title,
    ...
    question(title='Milk', message='Yes or No?') {
        return milk._call_api({
            event: 'dialog.question',
            params: {
                title,
                message
            }
    ...
    confirm(title='Milk', message='Are you sure you want to?') {
        return milk._call_api({
    ...
    open_file(title='Open File', initial_directory=null, file_types=null, multiselect=false) {
        return milk._call_api({
            event: 'dialog.open_file', 
            params: {
                title,
                initial_directory,
                file_types,
    ...
    save_file(suggested_filename, title='Save File', file_types=null) {
        return milk._call_api({
            event: 'dialog.save_file',
            params: {
                suggested_filename,
                title,
                file_types
    ...
    select_folder(title='Select Folder', initial_directory=null, multiselect=false) {
        return milk._call_api({
            event: 'dialog.select_folder',
            params: {
                title,
                initial_directory,
                multiselect
...
class Milk {
    ...
    async _connect_server() {
        this.ws = new WebsocketPromise({
            url: "ws://localhost:{MilkAppPort}/websocket"
        });
        await this.ws.connectionEstablished();
    }
    async _call_api(object) {
      const response = await this.ws.send(object);
      if (response.success) {
    ...
    _initial_api() {
        this.app = new App(this.ws);
        this.dialog = new Dialog(this.ws);
        this.debug = new Debug(this.ws);
    }
};

const milk = new Milk("{MilkAppPort}", "{MilkAppToken}");